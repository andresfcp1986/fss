...
(function () {
  ...
  // ── Acordeón Tabla de Contenidos ───────────────────────────
  const tocToggle = document.querySelector('.toc-accordion__toggle');
  const tocBody   = document.querySelector('.toc-accordion__body');
  ...
  if (tocToggle && tocBody) {
    tocToggle.addEventListener('click', () => {
      ...
      if (isOpen) {
        // Cerrar
        tocBody.setAttribute('hidden', '');
        tocToggle.setAttribute('aria-expanded', 'false');
      } else {
        // Abrir
        tocBody.removeAttribute('hidden');
        tocToggle.setAttribute('aria-expanded', 'true');
      }
    });

    // Los enlaces del TOC cierran el acordeón en móvil
