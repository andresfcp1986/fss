...
if (tocToggle && tocContent) {
  // Detecta si estamos en pantalla pequeña para el estado inicial
  const isMobile = window.matchMedia("(max-width: 768px)").matches;
  
  // En móvil empieza cerrado, en desktop abierto
  const initialState = isMobile ? "false" : "true";
  tocContent.setAttribute("data-open", initialState);
  tocToggle.setAttribute("aria-expanded", initialState);
  tocToggle.textContent = isMobile ? "Contenido ▼" : "Contenido ▲";

  tocToggle.addEventListener("click", () => {
    const isOpen = tocContent.getAttribute("data-open") === "true";
    tocContent.setAttribute("data-open", String(!isOpen));
    tocToggle.setAttribute("aria-expanded", String(!isOpen));
    tocToggle.textContent = !isOpen ? "Contenido ▲" : "Contenido ▼";
  });
}
